...
// PesaPal sends GET /api/payments/pesapal/ipn?orderTrackingId=...&orderMerchantReference=...
...
export async function markFeePaid(merchantRef: string, trackingId: string, confirmationCode: string) {
  // Try to find application via pesapal_merchant_ref column first
  let app: any = null;
  ...
  // Fallback: look up via transactions description
  if (!app) {
  ...
  // Mark fee paid — try with pesapal columns, fall back without
  try {
