...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Persist tracking id — wrapped in try/catch in case columns don't exist yet
    try {
    ...
    // Always store in transactions table as reliable fallback
    ...
    return sendResponse(200, true, 'Payment initiated', { redirect_url: redirectUrl });
  } catch (err: any) {
    console.error('PesaPal initiate error:', err);
    return sendResponse(500, false, err.message || 'Failed to initiate PesaPal payment');
  }
}
